...
```{r}
# calculate threshold as average probability of being retained
threshold <- mean(phat.lgfit.selected)

# use threshold to predict binary classification
pred.lgfit.selected <- ifelse(phat.lgfit.selected > threshold, 1, 0)
pred.l1.lgfit <- ifelse(phat.l1.lgfit > threshold, 1, 0)
```
...
```{r}
get_confusion_matrix(df.test$Retained.in.2012.,pred.lgfit.selected,threshold)
```
```{r}
get_confusion_matrix(df.test$Retained.in.2012.,pred.l1.lgfit,threshold)
```
   From the above confusion matrices, both the Regression and LASSO 
